...
part_int=part_int_back(and(part_int_back(:,1)>700,part_int_back(:,1)<2800/2),:);
...
k_tmp=[];
for iP=1:size(part_int,1);if and(max(part_int_tmp(iP,:))<20,min(part_int_tmp(iP,:))>0.1);pp2=polyfit((5:30)*10,polyval(pp,4./part_int_tmp2(iP,5:30)),1);k_tmp=[k_tmp pp2(1)];;end;end
...
for iP=1:size(part_int_tmp,1);
    if and(max(part_int_tmp(iP,:))<20,min(part_int_tmp(iP,:))>0.05);
        t_arr=[];
        I_arr=[];
