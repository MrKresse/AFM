...
i_lower=300;
i_upper=2500;
i_up=[607:612 614:618 620:628 630:633 635 637:639 642:653 655:671];
...
%sort median distances ascending
tmptmp=median(sd,2);
[iy,ii]=sort(tmptmp);

%recalculate distances in the sorted matrix
sd=0*ones(size(tmpX_all,2),size(tmpX_all,2));
for iX=1:size(tmpX_all,2)
...
%calculate log distance of distances
...
figure;for iX=[607:671];plot((1:size(tmpX_all,1))*dt,tmpX_all(:,ii(iX)),'-','Tag',num2str(tmpX_id_all(ii(iX))));title(num2str(iX));axis([0 70*dt 6 9.5]);pause;end
...
figure
% hist(k_H_tmp(k_H_tmp<40),21)
hist(log10(k_H_tmp(k_H_tmp>0)),14)