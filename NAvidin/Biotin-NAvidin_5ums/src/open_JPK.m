...
elseif(strcmp(extension,valid_extensions{2,1}))
    ...
    [folderPath,FileName,~]=fileparts(complete_path_to_afm_file);
    extractionFolder = fullfile(folderPath, 'ForceCurvesMatlabExtracted');
    if ~exist(extractionFolder, 'dir')
        mkdir(extractionFolder)
    end
    unzip(complete_path_to_afm_file, fullfile(extractionFolder, FileName));

    % Dynamically list the extracted directory contents
    InfoDir = dir(fullfile(extractionFolder, FileName));
    ...
    z=1;
   % Define the path to the 'segments' folder
    segmentsFolder = fullfile(extractionFolder, FileName, 'segments');

    % List the contents of the 'segments' folder
    InfoDir_Segments = dir(segmentsFolder);
    ...
    for i=1:size(folderToEval,2)
        Info_location=fullfile(extractionFolder,FileName,'segments',InfoDir_Segments(folderToEval(1,i)).name,'segment-header.properties');
        ...
        % Define the path to the 'channels' folder for the current segment
        channelsFolder = fullfile(extractionFolder, FileName, 'segments', ...
                                  InfoDir_Segments(folderToEval(1, i)).name, 'channels');
        
        % List the contents of the 'channels' folder
        Temp_InfoDir_Segments = dir(channelsFolder);
...
%         end
        ...
        for j=1:size(Temp_InfoDir_Segments,1)
            % Define the path to the specific file within 'channels'
            filePath = fullfile(extractionFolder, FileName, 'segments', ...
                                InfoDir_Segments(folderToEval(1, i)).name, 'channels', ...
                                Temp_InfoDir_Segments(j).name);
            
            % Open the file
            fid = fopen(filePath);
            %flag_temp=fread(fid,'int32');
            flag_temp = fread(fid, 'int32', 'ieee-be');  % or 'ieee-be' if the data is big-endian
            fclose(fid);
            temp=strsplit(Temp_InfoDir_Segments(j).name,'.');
