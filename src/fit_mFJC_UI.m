...
function fit_mFJC_UI()
    %% Define the directory with processed files
    path_processed = 'C:\Users\kr3ss\Desktop\AFM\processed\';
    ...
    % Parameter inputs
    uicontrol(controlPanel, 'Style','text','String','v (m/s):',...
    ...
    uicontrol(controlPanel, 'Style','text','String','L_c (m):',...
    ...
    uicontrol(controlPanel, 'Style','text','String','T (K):',...
    ...
    uicontrol(controlPanel, 'Style','text','String','l_{kuhn} (m):',...
    ...
    uicontrol(controlPanel, 'Style','text','String','k_{segment} (N/m):',...
    ...
    function startFitCallback(~,~)
        ...
        try
            [loading_rate, unbinding_force, z_model, F_model] = fit_mFJC(path_processed, ...
                selectedFile, min_sep, max_sep, L_cont, T_val, l_kuhn_val, k_seg_val, v, ax);
        catch ME
            errordlg(['Fit failed: ' ME.message]);
            return;
        end
        msgbox(sprintf('Fit complete!\nLoading rate: %g N/s\nUnbinding force: %g N', loading_rate, unbinding_force));
        currentFit.base_str = selectedFile;
        currentFit.parameters = [L_cont, l_kuhn_val, k_seg_val, T_val, v];
